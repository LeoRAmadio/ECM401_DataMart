...
\section{Modelo Lógico Relacional (OLTP)}
\label{sec:modelo_oltp}

O Modelo Lógico Relacional representa a base operacional de onde os dados do C-MAPSS seriam extraídos. Ele foi normalizado para a Terceira Forma Normal (3NF) para garantir a integridade e otimizar as transações. O script SQL que gera o OLTP pode ser encontrado na pasta 'bronze' dentro da pasta 'sql'. O diagrama, conforme gerado pelo MySQL Workbench, é apresentado na Figura \ref{fig:diagrama_oltp}.

\FloatBarrier

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth*4/5]{images/oltp.jpeg} 
    \caption{Modelo Lógico Relacional (OLTP) da Base Operacional C-MAPSS}
    \label{fig:diagrama_oltp}
\end{figure}

\FloatBarrier

\subsection{Tabelas do Modelo OLTP}

\begin{itemize}
    \item \textbf{TAB\_MOTOR\_OLTP:} Armazena dados estáticos do motor, identificado por sua chave natural (dataset\_id, unit\_nr).
    \item \textbf{TAB\_CONFIGURACAO\_OLTP:} Armazena as condições operacionais únicas (setting1, setting2, setting3).
    \item \textbf{TAB\_LEITURA\_CICLO\_OLTP:} Tabela de fatos transacionais que registra as leituras dos sensores em um ciclo específico, referenciando as tabelas de motor e configuração.
\end{itemize}
